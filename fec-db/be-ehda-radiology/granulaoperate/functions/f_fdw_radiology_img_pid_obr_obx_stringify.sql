/*
    Function: fdw_radiology_img_pid_obr_obx_stringify
    ...
    See also:
        <radiology_hl7>
*/
CREATE FUNCTION fdw_radiology_img_pid_obr_obx_stringify (_cid VARCHAR)
RETURNS TABLE (pid_3_1 character varying, obr_3_1 character varying, obr_20_1 character varying, obr_21_1 character varying, obr_24_1 character varying, obx_3_1 character varying, obx_5_2 character varying, obx_14_1 character varying, control_id character varying, company_id integer) AS $BODY$

BEGIN

RETURN QUERY EXECUTE
$$WITH x AS (SELECT control_id, hl7_msg AS hm FROM radiology_hl7 WHERE control_id = $1)
 SELECT CAST((xpath('/HL7Message/PID/PID.3/PID.3.1/text()', hm))[1] AS VARCHAR),
    ...
    CAST((xpath('/HL7Message/OBX/OBX.14/OBX.14.1/text()', hm))[1] AS VARCHAR),
    control_id,
...
ALTER FUNCTION fdw_radiology_img_pid_obr_obx_stringify(VARCHAR)
  OWNER TO fluance;
