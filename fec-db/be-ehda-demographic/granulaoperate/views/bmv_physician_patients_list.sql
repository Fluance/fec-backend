/*
	View: bmv_physician_patients_list

	View used to provide a list patients for a physician

	See also:
		<patient>, <visit>, <m_rd_opa_pcl>, <m_rd_opa_pt>, <m_rd_opa_pca>, <m_rd_opa_hs>, <m_rd_opa_at>, <m_rd_opa_fcl>, <m_rd_opa_pu>
*/

CREATE OR REPLACE VIEW bmv_physician_patients_list AS
 SELECT v.patient_id,
	p.firstname,
	p.lastname,
	p.maidenname,
	...
	financialclassdesc,
	patientunitdesc
 FROM patient p JOIN visit v ON (v.patient_id = p.id) JOIN visit_physician vp ON (v.nb = vp.visit_nb) JOIN physician ph ON (vp.attending_physician_id = ph.id AND ph.company_id = v.company_id)
...
ALTER TABLE bmv_physician_patients_list
  OWNER TO fluance;
